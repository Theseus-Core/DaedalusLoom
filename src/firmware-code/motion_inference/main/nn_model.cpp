...
typedef struct {
    ...
    // Single branch / default details
    int class_count;
    char class_names[NN_MODEL_MAX_CLASS_COUNT][NN_MODEL_CLASS_NAME_MAX_LEN];
    
    // Multi-branch details
    bool is_multi_branch;
    
    int action_class_count;
    char action_class_names[NN_MODEL_MAX_CLASS_COUNT][NN_MODEL_CLASS_NAME_MAX_LEN];
    char action_output_name[64];
    
    int person_class_count;
    char person_class_names[NN_MODEL_MAX_CLASS_COUNT][NN_MODEL_CLASS_NAME_MAX_LEN];
    char person_output_name[64];

    bool metadata_loaded;
    dl::Model *model;
    dl::TensorBase *input;
    dl::TensorBase *output;
    dl::TensorBase *person_output;
} model_slot_t;
...
static dl::TensorBase *g_person_output = nullptr;
static int g_active_class_count = 4;
static bool g_active_is_multi_branch = false;
static int g_active_action_class_count = 4;
static int g_active_person_class_count = 0;
...
static void set_default_class_names(model_slot_t *slot)
{
    ...
    slot->is_multi_branch = false;
    slot->action_class_count = DEFAULT_CLASS_COUNT;
    slot->person_class_count = 0;
    slot->action_output_name[0] = '\0';
    slot->person_output_name[0] = '\0';
    ...
    for (int i = 0; i < NN_MODEL_MAX_CLASS_COUNT; i++) {
        if (i < DEFAULT_CLASS_COUNT) {
            ...
            copy_truncated(slot->action_class_names[i], sizeof(slot->action_class_names[i]), DEFAULT_CLASS_NAMES[i]);
        } else {
            snprintf(slot->class_names[i], sizeof(slot->class_names[i]), "Class %d", i);
            snprintf(slot->action_class_names[i], sizeof(slot->action_class_names[i]), "Class %d", i);
        }
        snprintf(slot->person_class_names[i], sizeof(slot->person_class_names[i]), "Person %d", i);
    }
}
...
static bool load_model_metadata_from_json(model_slot_t *slot, const char *path)
{
    ...
    cJSON *is_multi = cJSON_GetObjectItemCaseSensitive(root, "is_multi_branch");
    bool is_multi_branch = cJSON_IsBool(is_multi) && cJSON_IsTrue(is_multi);
    slot->is_multi_branch = is_multi_branch;

    if (is_multi_branch) {
        cJSON *act_branch = cJSON_GetObjectItemCaseSensitive(root, "action_branch");
        cJSON *pers_branch = cJSON_GetObjectItemCaseSensitive(root, "person_branch");
        if (!cJSON_IsObject(act_branch) || !cJSON_IsObject(pers_branch)) {
            ESP_LOGW(TAG, "multi-branch metadata missing action_branch or person_branch: %s", path);
            cJSON_Delete(root);
            return false;
        }

        // Parse Action branch
        cJSON *act_labels = get_json_item_any(act_branch, "labels", "classes");
        cJSON *act_count_item = get_json_item_any(act_branch, "num_classes", "class_count");
        if (!cJSON_IsArray(act_labels)) {
            ESP_LOGW(TAG, "action_branch labels missing or invalid: %s", path);
            cJSON_Delete(root);
            return false;
        }
        int act_label_count = cJSON_GetArraySize(act_labels);
        int act_class_count = cJSON_IsNumber(act_count_item) ? act_count_item->valueint : act_label_count;
        if (act_class_count <= 0 || act_class_count > NN_MODEL_MAX_CLASS_COUNT || act_class_count != act_label_count) {
            ESP_LOGW(TAG, "action_branch class count mismatch/invalid: %s (%d, %d)", path, act_class_count, act_label_count);
            cJSON_Delete(root);
            return false;
        }
        slot->action_class_count = act_class_count;
        slot->class_count = act_class_count; // For backwards compatibility
        for (int i = 0; i < act_class_count; i++) {
            cJSON *label = cJSON_GetArrayItem(act_labels, i);
            if (cJSON_IsString(label) && label->valuestring) {
                copy_truncated(slot->action_class_names[i], sizeof(slot->action_class_names[i]), label->valuestring);
                copy_truncated(slot->class_names[i], sizeof(slot->class_names[i]), label->valuestring);
            }
        }
        for (int i = act_class_count; i < NN_MODEL_MAX_CLASS_COUNT; i++) {
            snprintf(slot->action_class_names[i], sizeof(slot->action_class_names[i]), "Class %d", i);
            snprintf(slot->class_names[i], sizeof(slot->class_names[i]), "Class %d", i);
        }
        cJSON *act_out = cJSON_GetObjectItemCaseSensitive(act_branch, "output_name");
        if (cJSON_IsString(act_out) && act_out->valuestring) {
            copy_truncated(slot->action_output_name, sizeof(slot->action_output_name), act_out->valuestring);
        } else {
            slot->action_output_name[0] = '\0';
        }

        // Parse Person branch
        cJSON *pers_labels = get_json_item_any(pers_branch, "labels", "classes");
        cJSON *pers_count_item = get_json_item_any(pers_branch, "num_classes", "class_count");
        if (!cJSON_IsArray(pers_labels)) {
            ESP_LOGW(TAG, "person_branch labels missing or invalid: %s", path);
            cJSON_Delete(root);
            return false;
        }
        int pers_label_count = cJSON_GetArraySize(pers_labels);
        int pers_class_count = cJSON_IsNumber(pers_count_item) ? pers_count_item->valueint : pers_label_count;
        if (pers_class_count <= 0 || pers_class_count > NN_MODEL_MAX_CLASS_COUNT || pers_class_count != pers_label_count) {
            ESP_LOGW(TAG, "person_branch class count mismatch/invalid: %s (%d, %d)", path, pers_class_count, pers_label_count);
            cJSON_Delete(root);
            return false;
        }
        slot->person_class_count = pers_class_count;
        for (int i = 0; i < pers_class_count; i++) {
            cJSON *label = cJSON_GetArrayItem(pers_labels, i);
            if (cJSON_IsString(label) && label->valuestring) {
                copy_truncated(slot->person_class_names[i], sizeof(slot->person_class_names[i]), label->valuestring);
            }
        }
        for (int i = pers_class_count; i < NN_MODEL_MAX_CLASS_COUNT; i++) {
            snprintf(slot->person_class_names[i], sizeof(slot->person_class_names[i]), "Person %d", i);
        }
        cJSON *pers_out = cJSON_GetObjectItemCaseSensitive(pers_branch, "output_name");
        if (cJSON_IsString(pers_out) && pers_out->valuestring) {
            copy_truncated(slot->person_output_name, sizeof(slot->person_output_name), pers_out->valuestring);
        } else {
            slot->person_output_name[0] = '\0';
        }
        ...
        ESP_LOGI(TAG, "loaded multi-branch metadata for %s: action: %d classes, person: %d classes from %s", 
                 slot->name, slot->action_class_count, slot->person_class_count, path);
    } else {
        // Single branch (original flow)
        ...
        if (class_count != label_count) {
            ESP_LOGW(TAG, "metadata class count mismatch: %s (%d labels, count=%d)",
                     path, label_count, class_count);
            cJSON_Delete(root);
            return false;
        }

        slot->class_count = class_count;
        slot->action_class_count = class_count;
        slot->person_class_count = 0;
        slot->action_output_name[0] = '\0';
        slot->person_output_name[0] = '\0';
...
static esp_err_t ensure_model_loaded(int model_id)
{
    ...
    if (slot->is_multi_branch) {
        // Resolve action output tensor
        if (slot->action_output_name[0] != '\0' && model_outputs.find(slot->action_output_name) != model_outputs.end()) {
            slot->output = model_outputs[slot->action_output_name];
        } else {
            slot->output = model_outputs.begin()->second;
        }

        // Resolve person output tensor
        if (slot->person_output_name[0] != '\0' && model_outputs.find(slot->person_output_name) != model_outputs.end()) {
            slot->person_output = model_outputs[slot->person_output_name];
        } else {
            if (model_outputs.size() >= 2) {
                auto it = model_outputs.begin();
                it++; // second item
                slot->person_output = it->second;
            } else {
                slot->person_output = nullptr;
            }
        }

        if (!slot->output || !slot->person_output) {
            ESP_LOGE(TAG, "%s multi-branch output tensors not found or incomplete", slot->name);
            delete slot->model;
            slot->model = nullptr;
            slot->input = nullptr;
            slot->output = nullptr;
        ...
        int action_out_count = infer_output_class_count(slot->output);
        int person_out_count = infer_output_class_count(slot->person_output);

        if (action_out_count <= 0 || person_out_count <= 0) {
            ESP_LOGE(TAG, "%s multi-branch outputs have invalid class counts: action=%d, person=%d",
                     slot->name, action_out_count, person_out_count);
            delete slot->model;
            slot->model = nullptr;
            slot->input = nullptr;
            slot->output = nullptr;
        ...
        if (slot->metadata_loaded) {
            if (slot->action_class_count != action_out_count || slot->person_class_count != person_out_count) {
                ESP_LOGE(TAG, "%s metadata class counts (act=%d, pers=%d) mismatch model outputs (act=%d, pers=%d)",
                         slot->name, slot->action_class_count, slot->person_class_count, action_out_count, person_out_count);
                delete slot->model;
                slot->model = nullptr;
                slot->input = nullptr;
                slot->output = nullptr;
            ...
            slot->action_class_count = action_out_count;
            slot->class_count = action_out_count;
            slot->person_class_count = person_out_count;
        }

        ESP_LOGI(TAG, "%s loaded (multi-branch). Size: %ld bytes, Action Classes: %d, Person Classes: %d",
                 slot->name,
                 slot->size,
                 slot->action_class_count,
                 slot->person_class_count);
    } else {
        slot->output = model_outputs.begin()->second;
        slot->person_output = nullptr;
        ...
        if (slot->metadata_loaded) {
            ...
            slot->action_class_count = output_class_count;
        }
...
esp_err_t nn_model_select(int model_id)
{
    ...
    g_person_output = slot->person_output;
    g_active_class_count = slot->class_count;
    g_active_is_multi_branch = slot->is_multi_branch;
    g_active_action_class_count = slot->action_class_count;
    g_active_person_class_count = slot->person_class_count;
    sys_config_set_active_model_id(model_id);
    if (g_active_is_multi_branch) {
        ESP_LOGI(TAG, "Active model switched to %s (multi-branch, action=%d classes, person=%d classes)", 
                 slot->name, g_active_action_class_count, g_active_person_class_count);
    } else {
...
bool nn_model_is_multi_branch(void)
{
    return g_active_is_multi_branch;
}

int nn_model_get_action_class_count(void)
{
    return g_active_action_class_count;
}

int nn_model_get_person_class_count(void)
{
    return g_active_person_class_count;
}

const char *nn_model_get_action_class_name(int class_id)
{
    if (class_id < 0 || class_id >= g_active_action_class_count || !is_valid_model_id(g_active_model_id)) {
        return "Unknown";
    }
    const char *name = g_model_slots[g_active_model_id].action_class_names[class_id];
    return name[0] != '\0' ? name : "Unknown";
}

const char *nn_model_get_person_class_name(int class_id)
{
    if (class_id < 0 || class_id >= g_active_person_class_count || !is_valid_model_id(g_active_model_id)) {
        return "Unknown";
    }
    const char *name = g_model_slots[g_active_model_id].person_class_names[class_id];
    return name[0] != '\0' ? name : "Unknown";
}
...
int nn_model_predict_cnn_multi(float *raw_csi, float *out_action_probs, float *out_person_probs, int *out_person_class)
{
    if (g_model == nullptr || g_model_input == nullptr || g_model_output == nullptr || g_person_output == nullptr) {
        ESP_LOGE(TAG, "Multi-branch model not initialized properly!");
        return -1;
    }
    if (raw_csi == nullptr || out_action_probs == nullptr || out_person_probs == nullptr || out_person_class == nullptr) {
        return -1;
    }
    ...
    // 1. Calculate Softmax probabilities for Action branch
    int act_count = g_active_action_class_count;
    if (act_count <= 0) {
        return -1;
    }
    float act_sum_exp = 0.0f;
    float act_logits[NN_MODEL_MAX_CLASS_COUNT];
    float act_max_logit = tensor_value_as_float(g_model_output, 0);
    for (int c = 1; c < act_count; c++) {
        float val = tensor_value_as_float(g_model_output, c);
        if (val > act_max_logit) {
            act_max_logit = val;
        }
    }
    for (int c = 0; c < act_count; c++) {
        act_logits[c] = tensor_value_as_float(g_model_output, c);
        act_sum_exp += expf(act_logits[c] - act_max_logit);
    }
    for (int c = 0; c < act_count; c++) {
        out_action_probs[c] = expf(act_logits[c] - act_max_logit) / act_sum_exp;
    }
    for (int c = act_count; c < NN_MODEL_MAX_CLASS_COUNT; c++) {
        out_action_probs[c] = 0.0f;
    }

    int max_action_class = 0;
    float max_action_prob = out_action_probs[0];
    for (int c = 1; c < act_count; c++) {
        if (out_action_probs[c] > max_action_prob) {
            max_action_prob = out_action_probs[c];
            max_action_class = c;
        }
    }

    // 2. Calculate Softmax probabilities for Person branch
    int pers_count = g_active_person_class_count;
    if (pers_count <= 0) {
        *out_person_class = -1;
        memset(out_person_probs, 0, NN_MODEL_MAX_CLASS_COUNT * sizeof(float));
    } else {
        float pers_sum_exp = 0.0f;
        float pers_logits[NN_MODEL_MAX_CLASS_COUNT];
        float pers_max_logit = tensor_value_as_float(g_person_output, 0);
        for (int c = 1; c < pers_count; c++) {
            float val = tensor_value_as_float(g_person_output, c);
            if (val > pers_max_logit) {
                pers_max_logit = val;
            }
        }
        for (int c = 0; c < pers_count; c++) {
            pers_logits[c] = tensor_value_as_float(g_person_output, c);
            pers_sum_exp += expf(pers_logits[c] - pers_max_logit);
        }
        for (int c = 0; c < pers_count; c++) {
            out_person_probs[c] = expf(pers_logits[c] - pers_max_logit) / pers_sum_exp;
        }
        for (int c = pers_count; c < NN_MODEL_MAX_CLASS_COUNT; c++) {
            out_person_probs[c] = 0.0f;
        }

        int max_person_class = 0;
        float max_person_prob = out_person_probs[0];
        for (int c = 1; c < pers_count; c++) {
            if (out_person_probs[c] > max_person_prob) {
                max_person_prob = out_person_probs[c];
                max_person_class = c;
            }
        }
        *out_person_class = max_person_class;
    }

    return max_action_class;
}
