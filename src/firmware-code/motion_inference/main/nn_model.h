...
/**
 * @brief Return whether the active model is a multi-branch model.
 */
bool nn_model_is_multi_branch(void);

/**
 * @brief Return the action branch class count for the active model.
 */
int nn_model_get_action_class_count(void);

/**
 * @brief Return the person branch class count for the active model.
 */
int nn_model_get_person_class_count(void);

/**
 * @brief Return the action branch class name for the active model.
 */
const char *nn_model_get_action_class_name(int class_id);

/**
 * @brief Return the person branch class name for the active model.
 */
const char *nn_model_get_person_class_name(int class_id);

/**
 * @brief Run inference on preprocessed 1D-CNN features for a multi-branch model.
 * 
 * @param raw_csi Pointer to 11400 floats
 * @param out_action_probs Pointer to float array of size NN_MODEL_MAX_CLASS_COUNT for action probabilities
 * @param out_person_probs Pointer to float array of size NN_MODEL_MAX_CLASS_COUNT for person probabilities
 * @param out_person_class Output parameter for predicted person class
 * @return int Predicted action class, or -1 on error
 */
